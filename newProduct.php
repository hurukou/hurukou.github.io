...
if(!empty($_POST)){
    ...
    if(empty($dbUserData)){
        ...
        validCategory($category, 'category_id');
    ...
    if(empty($err_msg)){
        ...
        try{
            ...
            //編集ならUPDATE,新規登録ならINSERT分
            if($edit_flg){
                $sql = 'UPDATE product SET title = :title, category_id = :category, price = :price, place = :place, content = :content, ask = :ask, pic = :pic WHERE user_id = :u_id AND id = :p_id';
                
                $data = array(':title' => $title , ':category' => $category, ':price' => $price, ':place' => $place, ':content' => $contet, ':ask' => $ask, ':pic' => $pic, ':u_id' => $_SESSION['user_id'], ':p_id' => $p_id);
            }else{
                $sql = 'insert into product (title, category_id, price, place, content, ask, pic, user_id, create_date ) values (:title, :category, :price, :place, :content, :ask, :pic, :u_id, :date)';
                $data = array(':title' => $title , ':category' => $category, ':price' => $price, ':place' => $place, ':content' => $content, ':ask' => $ask, ':pic' => $pic, ':u_id' => $_SESSION['user_id'], ':date' => date('Y-m-d H:i:s'));
            }
   ...
   <section id="main">
      <div class="form-container">
          <form action="" method="post" class="form" enctype="multipart/form-data" style="width:100%,box-sizing:border-box">
              ...
              <div>カテゴリー<span class="Required"></span>
              <select name="category_id" id="">
                  <option value="0" <?php if(formData('category_id') ==0 ){ echo 'selected'; } ?> >選択してください</option>
              <?php
                  foreach($dbCategory as $key => $val){       
              ?>
                  <option value="<?php echo $val['id'] ?>" <?php if(FormData('category_id') == $val['id'] ){ echo 'selected'; } ?> >
                      <?php echo $val['name']; ?>
                  </option>
              <?php      
              }
