...
define('MSG02', 'メールアドレスの形式でご入力してください');
define('MSG03', 'パスワード(再入力)があっていません');
define('MSG04', '半角英数字でご入力ください');
define('MSG05', '6文字以上でご入力ください');
define('MSG06', '256文字以内でご入力ください');
define('MSG07', 'エラーが発生しました。エラーが発生しました。しばらくしてからもう一度お試しください。');
...
function getProductInfo($p_id)
{
    ...
    try {
        $db = db_connection();
        $sql = 'SELECT p.id , p.title , p.content, p.ask, p.price, p.pic, p.user_id, p.create_date, p.update_date, c.name AS category 
